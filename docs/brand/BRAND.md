...
| [`keeper-terminal.png`](keeper-terminal.png) | Brand-styled "coincoin · keeper" terminal: watch → "COIN COIN ! threat" → "evacuated to SafeVault", the real live watcher log lines | Day 3, tweet 2: the automatic detect → evacuate loop |
| [`aave-fork-test.png`](aave-fork-test.png) | Brand-styled "AaveRealFork.t.sol" code card (exitAaveV3 against the real Aave V3 Pool) with a green "FORK-VERIFIED" stamp | Day 3, tweet 3: the DeFi position exit (Harpie blind spot) |
